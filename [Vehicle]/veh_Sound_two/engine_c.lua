...
ENGINE_ENABLED = true
ENGINE_VOLUME_MASTER = 0.2 -- volume multiplier
ENGINE_VOLUME_THROTTLE_BOOST = 1.1	 -- boosting engine volume if we use throttle
			...
			if engine then
				...
				if getVehicleEngineState(vehicle) == true and distance < ENGINE_SOUND_DISTANCE*2 then
					...
					if not data.reverse and velocity < 10 then
						data.reverse = controller and (getPedAnalogControlState(controller, "brake_reverse") > 0.5)
					...
					data.forceNeutral = isSkidding -- w / s or handbrake without moving: neutral gear
								or (isLineOfSightClear(x, y, z, x, y, z-(getElementDistanceFromCentreOfMassToBaseOfModel(vehicle)*2), true, false, false, true, true, false, false, vehicle) and data.throttle) -- vehicle in air: neutral gear
					...
					if not data.sounds then 
						...
						setSoundSpeed(data.sounds[2], (rpm/(engine.maxRPM*0.6))*(data.reverse and 0.5 or 1))
					...
					if DEBUG and vehicle == myVehicle then
						dxDrawText("Motor\nType: "..tostring(engine.name).."\nRPM: "..tostring(rpm).."\nVol1: "..tostring(svol[1]).."\nVol2: "..tostring(svol[2]).."\nVol3: "..tostring(svol[3]), 300, 300)
						
						local t = "Ratios\Gears: "..tostring(data.gear).."/"..tostring(#gearRatios).."\n"
