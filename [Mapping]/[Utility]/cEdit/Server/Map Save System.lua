...
-- # .CSP Objective
save.settings.objective = {
'Team',
'Obj Type',
'Obj ID'
}

functions.save['Objectives'] = function (saveFolder)
	save.Count['Objectives'] = 0
	for i,v in pairs(getElementsByType('object')) do
		if getElementData(v,'mID') and (getElementData(v,'eType') == 'Objective') then
			save.Count['Objectives'] = save.Count['Objectives'] + 1 
		end
	end
	if save.Count['Objectives'] > 0 then -- Only write if the spawn count is higher then 1
		local pFile = fileCreate(':'..saveFolder..'/CSP/Objectives.CSP')
		fileWrite(pFile, '0,0,0\n')
		for i,v in pairs(getElementsByType('object')) do
			if getElementData(v,'mID') and (getElementData(v,'eType') == 'Objective') then
				local id = getElementData(v,'mID')
				...
				sString = ''
				for ia,va in pairs(save.settings.objective) do
					local value = getElementData(v,va)
					if type(value) == 'table' then
						local r,g,b = unpack(value)
						sString = sString..','..math.floor(r)..','..math.floor(g)..','..math.floor(b)
		...
		if fileExists(':'..saveFolder..'/CSP/Objectives.CSP') then
			fileDelete(':'..saveFolder..'/CSP/Objectives.CSP')
		end
	end 
end
...
functions.save['Weapons'] = function (saveFolder)
	save.Count['Weapons'] = 0
	for i,v in pairs(getElementsByType('object')) do
		if getElementData(v,'mID') and (getElementData(v,'eType') == 'Weapon') then
			save.Count['Weapons'] = save.Count['Weapons'] + 1 
		end
	end
	if save.Count['Weapons'] > 0 then -- Only write if the spawn count is higher then 1
		local pFile = fileCreate(':'..saveFolder..'/CSP/Weapons.CSP')
		fileWrite(pFile, '0,0,0\n')
		for i,v in pairs(getElementsByType('object')) do
			if getElementData(v,'mID') and (getElementData(v,'eType') == 'Weapon') then
				local id = getElementData(v,'mID')
		...
		if fileExists(':'..saveFolder..'/CSP/Weapons.CSP') then
			fileDelete(':'..saveFolder..'/CSP/Weapons.CSP')
		end
	end 
end
