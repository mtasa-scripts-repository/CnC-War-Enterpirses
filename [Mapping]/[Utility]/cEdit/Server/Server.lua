...
lSecession = {cDefaults = {}} -- Secession Information
sSecession = {Elements = {}} -- Shared Information (Do not store elements)
functions = {}
duplicate = {} -- Used for copying special elements.
...
functions.getNumber = function()
	return '|'..math.random(0,1000)..math.random(0,1000)
end

functions.generateElementID = function (type)
	local number = functions.getNumber()
	local id = type..number
	
	if type == 'vehicle' then
		for i,v in pairs(getElementsByType('vehicle')) do
			if getElementData(v,'mID') == id then
				return functions.generateID(type)
			end
		end
	else
...
functions.setSelected = function (element,selected)
	if isElement(element) then
		setElementFrozen(element,selected or false)
	end
end
