...
functions.drawRightClickMenu = function () -- Only shows when you right click
	...
	if rightClickMenu or (rightClickMenuFade > 0) then
		...
		if isCursorShowing() then
			...
			if functions.isCursorOnElement(x,y,menus.settings.width*s,menus.settings.height*s,nil,'RCM') > 0 then
				...
				if getKeyState('mouse1') then
					...
					if ((x+menus.settings.width*s) > xSize) then
						currentRightClickMenu[1] = (xSize-menus.settings.width-(menus.settings.width/4))
					elseif ((x*s) < 0) then
						currentRightClickMenu[1] = (menus.settings.width/16)
					end
					
					if (y+menus.settings.height*s) > ySize then
						currentRightClickMenu[2] = (ySize-((menus.settings.height+1)*(rcIndex+3)))
					end
				end
				else
				rcmHover = nil
		...
		if rightClickSetting then
			rcIndex = rcIndex + 1
			dxDrawRectangle (x,y+(((menus.settings.height+1)*rcIndex)*s),menus.settings.width*s,menus.settings.height*s, tocolor ( 0, 0, 0, 150*rightClickMenuFade ),true )
			functions['Button']({'Button','Copy Setting',nil},x,y+(((menus.settings.height+1)*rcIndex)*s),menus.settings.width*s,menus.settings.height*s,'rightClick',1,true,rightClickMenuFade)
			if lSecession.rcCopy then
				if lSecession.rcCopy[2] == functions.findType() then
					rcIndex = rcIndex + 1
					dxDrawRectangle (x,y+(((menus.settings.height+1)*rcIndex)*s),menus.settings.width*s,menus.settings.height*s, tocolor ( 0, 0, 0, 150*rightClickMenuFade ),true )
					functions['Button']({'Button','Paste Setting',nil},x,y+(((menus.settings.height+1)*rcIndex)*s),menus.settings.width*s,menus.settings.height*s,'rightClick',1,true,rightClickMenuFade)
					
					rcIndex = rcIndex + 1
					dxDrawRectangle (x,y+(((menus.settings.height+1)*rcIndex)*s),menus.settings.width*s,menus.settings.height*s, tocolor ( 0, 0, 0, 150*rightClickMenuFade ),true )
					functions['Button']({'Button','Paste (Instance) Setting',nil},x,y+(((menus.settings.height+1)*rcIndex)*s),menus.settings.width*s,menus.settings.height*s,'rightClick',1,true,rightClickMenuFade)
				end
			end
			rcIndex = rcIndex + 1
			dxDrawRectangle (x,y+(((menus.settings.height+1)*rcIndex)*s),menus.settings.width*s,menus.settings.height*s, tocolor ( 0, 0, 0, 150*rightClickMenuFade ),true )
			functions['Button']({'Button','Uninstance Setting',nil},x,y+(((menus.settings.height+1)*rcIndex)*s),menus.settings.width*s,menus.settings.height*s,'rightClick',1,true,rightClickMenuFade)
		end
...
functions.onRightClick = function( button, state, x, y )
	if (button == 'right') and (state == 'down') then
		if rightClickMenu then
			rightClickMenu = nil
			rightClickSetting = nil
		else
			rightClickMenu = {x,y}
			local hover = (lSecession.description or {})[1]
			if lSecession.variables[hover] then
				rightClickSetting = hover
			end
		end
	end
