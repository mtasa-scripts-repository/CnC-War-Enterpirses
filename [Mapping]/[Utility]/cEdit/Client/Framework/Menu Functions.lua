...
functions['Option'] = function(arguments,x,y,w,h,side,_,_,fadePercent)
	...
	if image then
		...
		dxDrawText(functions.fetchTranslation(arguments[2]), x+(10*s)+(h*1.2),y,x+w,y+h, tocolor(255, 255, 255, 220*fadePercent), 0.8*s, "default-bold", "left", "center", false, false, true, false, false)
		else
		dxDrawText(functions.fetchTranslation(arguments[2]), x+(10*s),y,x+w,y+h, tocolor(255, 255, 255, 220*fadePercent), 0.8*s, "default-bold", "left", "center", false, false, true, false, false)
	end
...
functions.options = function (name,list,direction)
	if lSecession.variables[name] then
		...
		local new = ((lSecession.variables[name][2] or 0))+direction
		if new < 1 then
