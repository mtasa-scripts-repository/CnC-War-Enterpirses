...
functions.setSelected = function(element,noDeselect,fixBounding)
		...
		for index,e in pairs(lSecession.Selected) do
			if element == e then
				...
				if #lSecession.Selected == 0 then 
					...
					functions.prepCustomization.Main()
				return
			end
		end
	end
	...
	functions.server('setSelected',element,getElementData(element,'Selected'))
	functions.prepCustomization.Main()
	if fixBounding then
		functions.alignToFace(element)
	end
end

functions.alignToFace = function(element)
	local xa,ya,za,xb,yb,zb = getElementBoundingBox ( element )

	local x,y,z = getElementPosition(element)
	local hit, Hx, Hy, Hz,hitElement = functions.proccessLineOfSite ( x, y, z+10, x,y,z-10,element )
	if hit then
		mSet.Position(element,x,y,Hz-za+1)
	end
end
...
functions.proccessLineOfSite = function (x,y,z,xa,ya,za,ignore)
	return processLineOfSight (	x, y, z, xa, ya, za,true,true,false,true,false,false,false,false,ignore,true)
end
...
fRender.findHighLightedElement = function()
	...
	if not isCursorShowing() then
		...
		if hit then
			if getElementData(element,'mID') then
