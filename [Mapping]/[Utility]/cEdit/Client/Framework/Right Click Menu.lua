...
functions['Copy Setting'] = function ()
	lSecession.rcCopy = {}
	lSecession.rcCopy[1] = lSecession.variables[rightClickSetting]
	lSecession.rcCopy[2] = functions.findType()
end

functions['Paste Setting'] = function ()
	local tabl = {}
	for i,v in pairs(lSecession.rcCopy[1]) do
		tabl[i] = v
	end
	lSecession.variables[rightClickSetting] = tabl
end

functions['Paste (Instance) Setting'] = function ()
	lSecession.variables[rightClickSetting] = lSecession.rcCopy[1]
end

functions['Uninstance Setting'] = function ()
	local tabl = {}
	for i,v in pairs(lSecession.variables[rightClickSetting]) do
		tabl[i] = v
	end
	lSecession.variables[rightClickSetting] = tabl
end
...
functions.findType = function ()
	if rightClickSetting then
		local data = lSecession.variables[rightClickSetting]
		if #data == 1 then
			return type(data)
		else
			return 'color'
