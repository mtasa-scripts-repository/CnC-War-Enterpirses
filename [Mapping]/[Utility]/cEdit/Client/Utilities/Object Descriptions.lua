...
functions.getElementDescription = function(element)
	if isElement(element) then
		if (getElementData(element,'eType') == 'Light Zone') then
			return (tostring(getElementModel(element))..':'..(getElementData(element,'mID') or '')),functions.prepImage('Lighting')
		elseif (getElementData(element,'eType') == 'Spawn Point') then
			return (tostring(getElementModel(element))..':'..(getElementData(element,'mID') or '')),functions.prepImage('Player Spawn')
		elseif getElementType(element) == 'object' then
			return (tostring(getElementModel(element))..':'..(getElementData(element,'mID') or '')),functions.prepImage('Object')
		elseif getElementType(element) == 'vehicle' then
			return (tostring((lSecession.VehicleNames[tostring(getElementModel(element))] or {})[getElementData(element,'vVarient') or 'Default'])),functions.prepImage('Create Vehicle')
		end
	end
end
