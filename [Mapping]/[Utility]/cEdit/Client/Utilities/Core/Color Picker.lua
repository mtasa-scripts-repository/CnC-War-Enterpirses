...
mRender.colorPicker = function ()
	if lSecession.cPicker.Open then
		...
		if (not _) then
			lSecession.variables[cPname] = {r,g,b,nil,nil,nil,functions.rgb2hex(r,g,b),true,255,255,255}
			return
		end
...
functions.colorPickerTrigger = function( _,_,x, y,_,_,_,ignoreBar,click )
	...
	if lSecession.variables[lSecession.cPicker.Open] then
		...
		functions.changeEdit('R',math.floor(tonumber(rA) or 255))
		functions.changeEdit('G',math.floor(tonumber(gA) or 255))
		functions.changeEdit('B',math.floor(tonumber(bA) or 255))
	end
	cOldx = x
	cOldy = y
end
