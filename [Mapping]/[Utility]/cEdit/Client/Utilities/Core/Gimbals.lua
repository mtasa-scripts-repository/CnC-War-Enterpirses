...
functions.resetCircles = function()
	...
	functions.noCollideCircles()
end



functions.noCollideCircles = function()
	for i,v in pairs(getElementsByType('object')) do
		setElementCollidableWith(v,Gimbals.Circles[1],false)
		setElementCollidableWith(v,Gimbals.Circles[2],false)
		setElementCollidableWith(v,Gimbals.Circles[3],false)
	end
...
setTimer (functions.noCollideCircles,500,0)
...
lRender.drawRotationGimbal = function () -- Well flip me over and call me sally; this may be the hardest thing I've done in a while.
	if (lSecession.variables['Mode'] or {})[1] == 'Rotation' then
		if (#lSecession.Selected > 0) then
			local tabl2 = {}
			local x,y,z = functions.getSelectedElementsCenter()
