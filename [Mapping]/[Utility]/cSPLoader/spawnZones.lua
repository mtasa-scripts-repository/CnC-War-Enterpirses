...
altFunt.spawnPoint = function(resourceName)
	if fileExists(':'..resourceName..'/CSP/spawnPoints.CSP') then
		local File = fileOpen(':'..resourceName..'/CSP/spawnPoints.CSP')
		...
		for iA,vA in pairs(Proccessed) do
			if iA == 1 then
				...
				if not (SplitA[1] == '!') then
					...
					count = 5
					for i,v in pairs(spawnPointStuff) do
						if v[2] == 3 then
							count = count + 1
					...
					setElementData(object,'eType','Spawn Point')
