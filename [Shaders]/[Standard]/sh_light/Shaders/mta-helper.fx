...
{
    // Incase we have no normal inputted
    if (OutNormal.x == 0 && OutNormal.y == 0 && OutNormal.z == 0 ) {OutNormal = float3(0, 1, 0);}
		
	if (OutNormal.x >= 1) {OutNormal.x = 1;}
	if (OutNormal.x <= -1) {OutNormal.x = -1;}
	if (OutNormal.y >= 1) {OutNormal.y = 1;}
	if (OutNormal.y <= -1) {OutNormal.y = -1;}
	if (OutNormal.z >= 1) {OutNormal.z = 1;}
	if (OutNormal.z <= -1) {OutNormal.z = -1;}
}
